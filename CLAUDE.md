...
- **Pages**: `index.md` (home/bio), `projects.md` (project listing, `permalink: /projects/`), `experience.md` (`permalink: /experience/`), one page per project in `projects/*.md` (all use `without-toc`), and `resume.md` (embeds PDFs from `assets/pdf/resume/` with `<object>`).
...
The resume PDFs are checked in under `assets/pdf/resume/` with the date in the filename (e.g. `balanced_9-13-26.pdf`). When a new version replaces one, update the filename in the `<object data>` in `resume.md`. The comment there mentions a `Resume` git submodule, but `.gitmodules` is currently empty (the submodule was removed and is planned to be re-added), so the PDFs must be copied in manually for now.
